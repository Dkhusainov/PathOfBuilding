...
--	self.socketViewer = common.New("PassiveTreeView")
	...
	for index, slotName in ipairs(baseSlots) do
		local slot = common.New("ItemSlotControl", {"TOPLEFT",prevSlot,"BOTTOMLEFT"}, 0, 0, self, slotName)
		prevSlot = slot
		...
		if slotName:match("Weapon") then
			...
			local swapSlot = common.New("ItemSlotControl", {"TOPLEFT",prevSlot,"BOTTOMLEFT"}, 0, 0, self, slotName.." Swap", slotName)
			prevSlot = swapSlot
	...
	for _, node in ipairs(socketOrder) do
		local socketControl = common.New("ItemSlotControl", {"TOPLEFT",prevSlot,"BOTTOMLEFT"}, 0, 0, self, "Jewel "..node.id, "Socket", node.id)
	...
	-- Unique database
	self.controls.uniqueDB = common.New("ItemDBControl", {"TOPLEFT",self.controls.itemList,"BOTTOMLEFT"}, 0, 76, 360, function(c) return m_min(244, self.maxY - select(2, c:GetPos())) end, self, main.uniqueDB[build.targetVersion], "UNIQUE")
	...
	self.controls.rareDB = common.New("ItemDBControl", {"TOPLEFT",self.controls.itemList,"BOTTOMLEFT"}, 0, 76, 360, function(c) return m_min(260, self.maxY - select(2, c:GetPos())) end, self, main.rareDB[build.targetVersion], "RARE")
